...
for (const document of documents) {
	...
	rows.push(await benchmark({
		...
		warmup,
		fn() {
			const result = turtleReferenceParser(document.turtle, url, 'text/turtle')
			if (!result.quads.length) {
				throw new Error(`reference parser produced no quads for ${document.name}`)
			}
		}
	}))
	...
	rows.push(await benchmark({
		...
		warmup,
		fn() {
			const context = oldm({parser: turtleReferenceParser})
			const source = context.parse(document.turtle, url, 'text/turtle')
			if (!source.data?.length) {
				throw new Error(`reference parser OLDM parse produced no data for ${document.name}`)
			}
		}
	}))
...
console.log('\nNotes')
console.log('- The recognition-only Ohm row measures parser-engine speed without CST walking or OLDM quad creation.')
console.log('- The parse-to-quads rows compare end-to-end Turtle-to-quad parser adapters, without OLDM object mapping.')
