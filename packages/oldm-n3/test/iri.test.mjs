...
import {n3Parser as parser, n3Writer as writer} from '@muze-nl/oldm-n3'
...
tap.test('N-Triples output keeps absolute IRIs and has no Turtle directives', async t => {
	const source = context().parse(`<${base}> <https://schema.org/url> <${base}/child> .`, base, 'application/n-triples')
	const output = await source.write()
	t.notMatch(output, /@base|@prefix/)
	t.match(output, `<${base}> <https://schema.org/url> <${base}/child> .`)
	t.equal(n3Parser(output, base, 'application/n-triples').quads[0].object.id, base+'/child')
	t.end()
})
